import titles_ideas.utils as utils
...
def titles_generation(insights_p1, insights_p2, insights_p3, channel_name):
    language = utils.get_titles_language()
    prompt_p4 = generate_phase4_prompt(insights_p1, insights_p2, insights_p3, channel_name, language)   
    title_ideas = analyze_with_gemini(prompt_p4)
    ...
    title_ideas_path = utils.export('title_ideas', title_ideas)
...
def transcripts_analysis(most_viewed_videos, channel_name):
    ...
    for video in videos:
        ...
        analysis += analyze_with_gemini(prompt_p3)
        analysis += "\n"
    
    prompt_p3 = generate_phase3_merge_prompt(analysis, most_viewed_videos_qty, channel_name)
    insights_p3 = analyze_with_gemini(prompt_p3)
    
    if not insights_p3:
        print("Failed to get insights from Phase 3.")
        return None
    
    insights_p3_path = utils.export('insights_p3', insights_p3)
...
def comments_analysis(most_viewed_videos):    
    ...
    insights_p2 = analyze_with_gemini(prompt_p2)
    
    if not insights_p2:
        print("Failed to get insights from Phase 2.")
        return None
    
    insights_p2_path = utils.export('insights_p2', insights_p2)
...
def channel_analysis(channel_name, channel_description, videos_list):    
    ...
    insights_p1 = analyze_with_gemini(prompt_p1)
    
    if not insights_p1:
        print("Failed to get insights from Phase 1.")
        return None
    
    insights_p1_path = utils.export('insights_p1', insights_p1)
...
def run_full_analysis_pipeline():
    ...
    channel_url = utils.get_youtube_channel_url()
    ...
    if insights_p1 and insights_p2 and insights_p3:
        ...
        titles_ideas = titles_generation(insights_p1, insights_p2, insights_p3, channel_name)
    else:
