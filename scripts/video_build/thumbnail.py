...
def run(image_path: str, output_path: str, default_path: str, channel_id: str):
    try:
        print(f"\t\t-Creating Thumbnail...")
        background = Image.open(image_path).convert("RGBA")
        width, height = background.size

        thumbnail_data_path = f"{default_path}/thumbnail_data.json"
        with open(thumbnail_data_path, "r", encoding="utf-8") as file:
