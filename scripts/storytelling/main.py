from scripts.utils import get_gemini_model, analyze_with_gemini, export, format_json_response, get_variables
...
def get_prompt(channel_id, step, file):
    file = f"storage/prompts/{channel_id}/{step}/{file}.txt"
    ...
    for title_id, title in enumerate(titles):
        ...
        if full_script:
            ...
            if not has_image_prompt:
                ...
                prompt = build_template(infos_variables, step='prompts',file_name="thumbnail")
                thumbnail_prompt = analyze_with_gemini(prompt)
            ...
            if not has_description:
                print(f"\t\t - Description...")
                prompt = build_template(infos_variables, step='prompts',file_name="description")
