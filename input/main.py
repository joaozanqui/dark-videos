...
def get_prompt(title: str, script: str) -> str:
    prompt = f"""
    ...
    **Your Input:** You will be provided with the title and the complete narration script of a video.
    ...
    **Your Mission:**
    ...
    1.  **Analyze and Synthesize:** Read the title and the entire script to deeply understand the core message, central theme, main subject, and overall mood. Do not just pick one scene. Your goal is to capture the *essence* of the entire narrative.
...
def run(title: str):
    ...
    title = "7 Passos para Libertar-se de Feridas Emocionais 💔"

    prompt = get_prompt(title, full_text)

    image_prompt = utils.analyze_with_gemini(prompt)
    prompt_path = utils.export('prompt', image_prompt, path='storage/images/')
    return prompt_path